describe("Prototype.Model", function() {
	describe("Basic", function() {
		it("defines the Prototype.Model class", function() {
			expect(Prototype.Model).toBeDefined();
		});

		describe("#idAttribute", function() {
			it("defines an IdAttribute class var which defaults to 'id'", function() {
				expect(Prototype.Model.prototype.idAttribute).toEqual('id');
			});
			...
			it("is possible to override the idAttribute for the whole class", function() {
				var M2 = Class.create(Prototype.Model, {
					idAttribute: 'myId'
				});
				var m1 = new Prototype.Model();
				var m2 = new M2();
		...
		describe("#constructor", function() {
			it("should set the delivered data as attributes", function() {
				spyOn(Prototype.Model.prototype, 'set');
				var a = new Prototype.Model({
					id: 4,
					name: 'Alex',
					active: true
				});

				expect(Prototype.Model.prototype.set).toHaveBeenCalledWith({
					id: 4,
					name: 'Alex',
					active: true
				});
		...
		describe("#getId", function() {
			it("exists", function() {
				expect(Prototype.Model.prototype.getId).toEqual(jasmine.any(Function));
			});
